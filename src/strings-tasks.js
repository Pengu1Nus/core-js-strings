...
function formatTime(minutes, seconds) {
  const minutesToStr = String(minutes);
  const secondsToStr = String(seconds);
  const formattedMinutes = minutesToStr.padStart(2, 0);
  const formattedSeconds = secondsToStr.padStart(2, 0);
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (vowelsList.includes(updatedStr[i])) {
      counter += 1;
    }
  }
  return counter;
...
function isPalindrome(str) {
  const changedStr = str.toLowerCase().replace(/[\W_]/g, '');
...
function findLongestWord(sentence) {
  ...
  let wordLargestLength = 0;
  let res = 0;
  for (let i = 0; i < sentenceArr.length; i += 1) {
    if (sentenceArr[i].length > wordLargestLength) {
      wordLargestLength = sentenceArr[i].length;
      res = i;
    }
  }
  return sentenceArr[res];
}
...
function reverseWords(str) {
  const reveredLetters = str.split('').reverse().join('');
  const reversedWords = reveredLetters.split(' ').reverse().join(' ');
...
function extractNameFromTemplate(value) {
  const solittedWords = value.split(', ');
  const res = solittedWords[1].slice(0, -1);
  return res;
}
